<template>
	<el-card>
		<div class="comment">
			<div class="title" v-if="m.type==1" >
				...
				<div class="title-right">
					...
					<el-button v-if="m.messageStatus=='未读'" type="success" icon="el-icon-check" circle size="mini" @click="confirm(m.id)"></el-button>				
					</div>
				</div>
				</div>
		...
		<div class="main">
			...
			<a :href="'/#/article/'+m.article.id"><div class="article"><p>来自文章:&ensp;{{m.article.title}}</p></div></a>
			<div v-show="!fold" class="showreply">
				<el-input class="reply" type="textarea" placeholder="输入你的回复" 
				v-model="reply.commentContent" resize="none">
				</el-input>
				<el-button @click="recallcomment(m.article.id,m.fromUser.id,m.commentId,m.id)" round size="mini" style="margin:5px;width:60px;">回复</el-button>
			</div>
			<span style="font-size: 13px;color:#9d9d9d;float: right;display: block;margin:8px;">{{m.gmtCreate}}</span>
		</div>
		</div>
	</el-card>
</template>
...
<script>
	import {recall,confirmmes} from'../../api/article.js'
	export default{
		name:'mesitemmain',
		props:{
			m:Object
